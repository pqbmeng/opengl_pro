...
const vec3 texMin = vec3(0,0,0);	//minimum texture access coordinate
const vec3 texMax = vec3(1,1,1);	//maximum texture access coordinate
...
{ 
	...
	vec3 geomDir = normalize((vUV-vec3(0.5,0.5,0.5)) - camPos); 
	...
	for (int i = 0; i < MAX_SAMPLES; i++) {
		...
		float r = prev_alpha * sample + vFragColor.r;
		vFragColor.r = r;
		vFragColor.g = r;
		vFragColor.b = r;
	...
	/*float sample_ = texture(volume, vUV-vec3(myoffset,myoffset,myoffset)).r;
	vFragColor =  vec4(sample_,sample_,sample_,sample_);
	if (0.1>sample_)
	{
